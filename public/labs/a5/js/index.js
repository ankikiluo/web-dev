...
// const four = numberArray1.find(a => a === 9);
// undefined
...
const init = () => {
    ...
    // it can also be used to modify its value.
    const setClassAttr = $("#set-class-attr");
    setClassAttr.attr('class', 'class-0');

    // Adding and Removing Classes
    // adds class class-1 to the heading
    const addClass1Example = $("#add-class-1");
    addClass1Example.addClass('class-1');

    // removeds the class class-2
    ...
    // 在child2的parents array中找到id="parent"的，selector赋值给parent1
    // 设置parent1的两个css
    parent1.css('background-color', 'red').css('color', 'white');

    // 把id="parent"的selector赋值给parent
    const parent = $("#parent");
    // 在parent的children中找到id="chils-2"的，selector赋值给child
    ...
    const handleEventTarget = (event) => {
        ...
        // target = <h2 id="event-target">Event target</h2>
        console.log(target);
        const getTarget = $(target);
        getTarget.css('background-color', 'blue').css('color', 'white');
    }
